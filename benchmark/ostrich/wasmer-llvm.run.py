...
def run(target):
    ...
    command_naive = ['wasmer', 'run', '--llvm', wasm_module_naive]
    command_opt = ['wasmtime', 'run', '--llvm', wasm_module_opt]
    command_simd = ['wasmtime', 'run', '--llvm', '--enable-simd', wasm_module_simd]
